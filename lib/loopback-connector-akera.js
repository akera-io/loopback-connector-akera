...
var AkeraConnector = function AkeraConnector(cfg) {
  ...
  this.all = function(model, filter, callback) {
    ...
    var query = self.connection.query.select(model);
    ...
    try {
      if (filter) {
        if (!filter.where)
          setFilter(query, {
            where : filter
          }, akeraApi.query.constants.action.select);
        else
          setFilter(query, filter, akeraApi.query.constants.action.select);
      }
  ...
  this.create = function(model, data, callback) {
    try {
      ...
      var query = self.connection.query.insert(model);
  ...
  this.destroyAll = function(model, filter, options, callback) {
    var query = self.connection.query.destroy(model);
  ...
  this.update = function(model, filter, data, callback) {
    var query = self.connection.query.update(model);
   ...
   *         callback The callback function
   */
  this.updateOrCreate = this.save = function(model, data, filter, callback) {
    var query = self.connection.query.upsert(model);
